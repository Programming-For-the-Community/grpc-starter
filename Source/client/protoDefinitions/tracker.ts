...
export function trackerStatusFromJSON(object: any): TrackerStatus {
  switch (object) {
    ...
    case 407:
    case "USER_NOT_MOVED":
      return TrackerStatus.USER_NOT_MOVED;
    case -1:
    case "UNRECOGNIZED":
    default:
...
export function trackerStatusToJSON(object: TrackerStatus): string {
  switch (object) {
    ...
    case TrackerStatus.USER_NOT_MOVED:
      return "USER_NOT_MOVED";
...
export const User: MessageFns<User> = {
  encode(message: User, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    globalThis.Object.entries(message.pathsTraveled).forEach(([key, value]: [string, Path]) => {
  ...
  fromJSON(object: any): User {
    return {
      ...
      pathsTraveled: isObject(object.pathsTraveled)
        ? (globalThis.Object.entries(object.pathsTraveled) as [string, any][]).reduce(
          (acc: { [key: number]: Path }, [key, value]: [string, any]) => {
  ...
  toJSON(message: User): unknown {
    ...
    if (message.pathsTraveled) {
      const entries = globalThis.Object.entries(message.pathsTraveled) as [string, Path][];
      if (entries.length > 0) {
        obj.pathsTraveled = {};
        entries.forEach(([k, v]) => {
  ...
  fromPartial<I extends Exact<DeepPartial<User>, I>>(object: I): User {
    ...
    message.currentLocation = (object.currentLocation !== undefined && object.currentLocation !== null)
      ...
      : undefined;
    message.pathsTraveled = (globalThis.Object.entries(object.pathsTraveled ?? {}) as [string, Path][]).reduce(
      (acc: { [key: number]: Path }, [key, value]: [string, Path]) => {
        if (value !== undefined) {
